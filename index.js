...
function hoursWorkedOnDate(date){
    const timeIn = this.timeInEvents.find(event => date)
...
function calculatePayroll(employeeRecords){
    let currentValue = 0
    return employeeRecords.map(employee => allWagesFor.call(employee)).reduce((total, currentValue) => total + currentValue, 0)
}
